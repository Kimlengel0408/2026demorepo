...
function add(num1, num2){ // this would add
    if (testing(num1, num2) != false){
        return num1 + num2 + ", the test passed";
    }
...
function nameCall(name){ // this would concatenate two strings
    return "Hello" + " " + name + ", the test passed";

}

function testing(n1, n2){
    if (n1 <= 0 || n2 <= 0) {
        throw new Error('Test Failed: Number entered is zero or negative value.');
        return false;
    } else if (!Number.isInteger(n1) || !Number.isInteger(n2)){
        throw new Error('Test Failed: Number entered is not Whole Number');
        return true;
    }
    return true;
